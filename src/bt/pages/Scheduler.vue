<template>
    <div>
      <q-page padding>
        ...
        <div class="q-mb-md">
          <q-btn color="red" @click="stopScheduler" v-if="schedulerStatus == 'running'" label="Stop Scheduler" :disable="schedulerStatus !== 'running'"/>
          <q-btn color="green" @click="startScheduler" v-if="schedulerStatus != 'running'" label="Start Scheduler" :disable="schedulerStatus === 'running'"/>
          <q-chip :color="schedulerStatus === 'running' ? 'green' : 'red'" text-color="white">
            {{ schedulerStatus.toUpperCase() }}
          </q-chip>
        </div>
        <job-table :jobs="jobs" @pause-job="pauseJob" @resume-job="resumeJob"></job-table>
      </q-page>
    </div>
  </template>
